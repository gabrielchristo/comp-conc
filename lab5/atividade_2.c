...
#define e_threads 5 // numero de threads escritoras
...
void entra_leitura(int id)
{
	...
	while (escr > 0 || escr_quer_prioridade > 0) {
		if(escr > 0 && escr_quer_prioridade == 0)
			printf("L[%d] bloqueou devido thread escrevendo\n", id);
		else if (escr == 0 && escr_quer_prioridade >= 0)
			printf("L[%d] bloqueou devido prioridade\n", id);
		else
			printf("L[%d] bloqueou devido thread escrevendo e prioridade\n", id);
