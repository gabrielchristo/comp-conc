/*
	Computação Concorrente - Problema leitor escritor
...
#define e_threads 3 // numero de threads escritoras
...
int foi_lido = 0; // apos leitura prioridade vai para escrita
int foi_escrito = 0; // apos escrita prioridade vai para leitura
...
void entra_leitura(int id)
{
	...
	while (escr > 0 || foi_lido == 1) {
		if(escr > 0 && foi_lido == 0)
			printf("L[%d] bloqueou devido thread escrevendo\n", id);
		else if(escr == 0 && foi_lido == 1)
			printf("L[%d] bloqueou devido prioridade para escrita\n", id);
		else
			printf("L[%d] bloqueou devido thread escrevendo e prioridade para escrita\n", id);
...
void entra_escrita(int id)
{
	...
	while (leit > 0 || foi_escrito == 1) {
		if(foi_escrito == 1 && leit == 0)
			printf("E[%d] bloqueou devido prioridade para leitura\n", id);
		else
			printf("E[%d] bloqueou devido thread lendo\n", id);
